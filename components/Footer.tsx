...
const SocialIcon: React.FC<{ href: string; 'aria-label': string; children: React.ReactNode }> = ({ href, 'aria-label': ariaLabel, children }) => (
  <a href={href} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-brand-gold transition-colors duration-300" aria-label={ariaLabel}>
    {children}
  </a>
);
...
const Footer: React.FC = () => {
  return (
    <footer id="contact" className="bg-black py-10 border-t border-brand-gray">
      <div className="container mx-auto px-6 text-center text-gray-400">
        ...
        <p className="text-sm">
          &copy; ${new Date().getFullYear()} Chronos Concept. Todos os direitos reservados.
        </p>
        <p className="text-xs mt-2 text-gray-500">
          Um portfólio conceitual. Relógios e imagens são para fins demonstrativos.
        </p>
      </div>
    </footer>
  );
